...
export default defineConfig({
  plugins: [react()],
  /**
   * `@ainize/core` resolves to its barrel (dist/index.js), which pulls ain-js and `node:module`'s createRequire into
  ...
  resolve: {
    alias: {
      ...
      '@ainize/core': fileURLToPath(new URL('../core/src/types.ts', import.meta.url)),
    },
  },
  server: {
    port: 5173,
