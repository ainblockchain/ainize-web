...
### 수업 하나가 얼마나 걸리는지, 안 끝날 때 무엇을 돌릴지

120문항 수업, 트레이너에 40GB A100 한 장 기준으로 측정했습니다.

| 무엇 | 얼마나 |
|---|---|
| 모델 올리기 | 5분. 수업 크기와 무관 |
| 가르치기 전 질문 다 물어보기 | 82분 — 문항당 약 40초 |
| 학습 1회 | 14분 |
| **평가 1회** | **88분 — 질문을 처음부터 다시 다 묻는 값** |

**수업이 끝나느냐를 정하는 것은 평가입니다.** 모든 질문을 다시 묻기 때문에 처음 한 바퀴와 같은 값이 들고,
기본값은 학습 2회마다 한 번입니다. 20회 학습이면 평가가 10번, 다섯 시간이 안 되는 학습 둘레에 열네 시간의
측정이 붙습니다.

설정 셋이고, 중요한 것은 첫 번째입니다.

```bash
ainize config set teach.effort.balanced.evalEvery 12   # 끝에 한 번만 측정
ainize config set teach.effort.balanced.maxSteps 12    # 이 데이터셋이 더 안 좋아지던 지점
ainize config set teach.trainer.timeoutMs 43200000     # 12시간. 기본 30분으로는 아무것도 못 끝냅니다
```

메모리가 모자라 죽는다면 손잡이는 `teach.trainer.microBatch`(여기선 8이 40GB 카드에 맞습니다. 기본 유도값은 큰
수업에 64를 줍니다)와 `teach.trainer.maxContrast`입니다. 두 번째를 낮추는 건 공짜 여유가 아니라 실제 거래입니다.
contrast는 이 수업이 무관한 답을 망가뜨리지 않게 지켜 주는 장치입니다.

**설정 파일이 아니라 노드가 실제로 쓴 값을 확인하세요.** 노드가 쓴 작업 명세가
`<runtime.repo>/.teach/<job-id>/job.json`에 있고 `micro`, `max_contrast`, `max_steps`, `eval_every`가 들어 있습니다.
