...
source_sha256: 5d534cb43981a4dd8a4c497dec793d39b025535afe18ea5121860a402642bc1a
---
...
`host`는 HTTP 서버가 실제로 붙는 인터페이스입니다. 기본값은 `127.0.0.1`, 곧 이 컴퓨터뿐입니다 — 새로 만든 노드는
따로 말하기 전까지 바깥에 열리지 않습니다. `ainize init --public`을 주면 모든 인터페이스인 `0.0.0.0`에 붙고, 아래 노드가
그렇게 시작됐기 때문에 넓은 값이 읽힙니다.

넓게 여는 것은 방화벽이나 프록시 뒤에서만 하세요. 운영자 API는 비밀번호가 아니라 서명으로 지켜지므로 이 노드가
인정한 키 없이는 아무도 들어오지 못합니다. 다만 공개된 것은 공개된 것이고, 포트는 누구나 닿습니다.
