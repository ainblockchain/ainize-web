...
export const GLOSSARY = {
  ...
  branch: { ko: '트랙', en: 'knowledge track', tech: 'branch (context-mapped patch set)', help_ko: '서로 다른 전제(예: 한국법 vs 미국법)를 따로 유지하는 트랙입니다. 노드는 트랙을 구독해 그 지식을 모델에 올려 두고, 게이트웨이는 상황에 맞는 트랙을 구독 중인 노드로 요청을 보냅니다.', help_en: 'Sets of knowledge kept in parallel for different contexts (e.g. KR law vs US law). A node subscribes to a track and keeps its knowledge loaded; the gateway sends a request to a node that subscribes to the fitting one.' },
  ...
  lineage: { ko: '계보', en: 'creator revenue share', tech: 'lineage royalties (parent patches)', help_ko: '다른 지식을 바탕으로 만든 지식이 팔리면 원작자에게도 자동으로 수익이 나뉩니다.', help_en: 'When knowledge built on someone else’s work sells, the original creator automatically gets a share.' },
  ...
  signedResult: { ko: '서명된 검증 결과', en: 'signed verification', tech: 'attestation signed with the verifier node key — no bond is escrowed anywhere', help_ko: '검증 노드가 자기 노드 키로 서명해 공개 기록에 남긴 결과입니다. 걸어 둔 보증금은 없습니다. 대신 누구나 이의를 제기할 수 있고, 이의가 제기되면 재검증될 때까지 판매가 멈춥니다.', help_en: 'A result the verifier node signed with its own key and left on the public record under that identity. No deposit is escrowed or slashed anywhere; instead any node can challenge a result, which takes the knowledge off sale until it is re-verified.' },
