...
export const operatorCommon: Dict = {
  ...
  'op.error.what.branches': { ko: '트랙 목록', en: 'the knowledge tracks' },
};
...
export const operatorDash: Dict = {
  ...
  'op.dash.branches.desc': { ko: '서로 다른 전제의 지식(예: 한국법 vs 미국법)을 트랙으로 따로 관리합니다. 구독하면 이 노드가 그 트랙의 “현재” 지식(교체된 지난 판은 제외)을 사서 모델에 넣고, 그 뒤로도 새로 올라오는 지식을 자동으로 받아 최신 상태를 유지합니다. 게이트웨이는 요청 상황에 맞는 트랙을 구독 중인 노드로 연결합니다 — 한 노드가 요청마다 트랙을 바꿔 끼우지는 않습니다.', en: 'Keep knowledge with different premises (e.g. KR law vs US law) in separate tracks. Subscribing buys the track’s CURRENT knowledge (not the versions it has replaced), loads it into this node’s model, and keeps up with whatever the track publishes next. The gateway routes a request to a node that subscribes to the fitting track — one node does not swap tracks per request.' },
  ...
  'op.dash.branches.earned': { ko: '이 트랙의 지식 판매 {n}건 · {total}', en: '{n} sale(s) of this track’s knowledge · {total}' },
  ...
  'op.dash.branches.empty': { ko: '아직 트랙이 없습니다.', en: 'No tracks yet.' },
  ...
  'op.dash.branches.cost.unknown': { ko: '이 트랙의 가격을 확인하지 못했습니다.', en: 'Could not price this track.' },
  ...
  'op.dash.branches.plan.retired': { ko: '이 트랙의 새 판으로 교체된 지난 판 — 사지도, 넣지도 않습니다', en: 'replaced by a newer version on this track — not bought, not loaded' },
  ...
  'op.dash.branches.sheet.sub': { ko: '구독은 이 트랙의 지식을 하나씩 구매해 실행 중인 모델에 넣습니다. 무엇을 얼마에 사는지 먼저 확인하세요.', en: 'Subscribing buys the track’s knowledge item by item and loads each one into the running model. Check what is bought, and for how much, first.' },
  'op.dash.branches.sheet.items': { ko: '이 트랙의 지식 목록', en: 'this track’s items' },
  ...
  'op.dash.branches.sheet.why.keepsup': { ko: '구독한 뒤에는 이 트랙에 새 지식이 올라올 때마다 이 노드가 자동으로 구매해 모델에 넣고 지난 판을 빼냅니다. 구독을 해지할 때까지 계속됩니다.', en: 'Once subscribed, this node buys and loads whatever the track publishes next and unloads the version it replaces — automatically, until you unsubscribe.' },
  'op.dash.branches.sheet.why.route': { ko: '구독이 성립한 노드만 이 트랙의 적용 조건에 맞는 요청을 받는 후보로 네트워크에 광고됩니다.', en: 'Only a node whose subscription actually went through is advertised to the network as serving this track’s context.' },
  'op.dash.branches.sheet.why.refund': { ko: '구독을 해지하면 이 트랙의 지식이 모델에서 빠집니다. 환불은 없습니다.', en: 'Unsubscribing takes the track’s knowledge out of the model. Nothing is refunded.' },
  'op.dash.branches.sheet.why.allornothing': { ko: '구독은 트랙 전체를 받습니다. 하나만 고를 수는 없으며, 하나만 원하면 그 지식의 페이지에서 직접 구매하세요.', en: 'Subscribing takes the whole track — there is no per-item choice here. For a single item, buy it from its own page.' },
  ...
  'op.dash.branches.unsub.model': { ko: '이 트랙의 지식 중 지금 모델에 들어가 있는 것을 모두 빼냅니다 — 이 노드의 답이 다시 달라집니다.', en: 'Every one of this track’s knowledge that is loaded into the running model is taken back out — this node’s answers change back.' },
  ...
  'op.dash.branch.create': { ko: '트랙 만들기', en: 'Create a track' },
  'op.dash.branch.name': { ko: '트랙 이름', en: 'Track name' },
  'op.dash.branch.desc': { ko: '설명', en: 'Description' },
  'op.dash.branch.desc.ph': { ko: '대한민국 관할 법률 트랙', en: 'Korean-jurisdiction legal knowledge' },
  'op.dash.branch.ctx': { ko: '적용 조건 (요청이 이 조건과 맞으면 이 트랙이 선택됩니다)', en: 'Match conditions (requests matching these pick this track)' },
  ...
  'op.dash.branch.submit': { ko: '트랙 만들기', en: 'Create track' },
  'op.dash.branch.creating': { ko: '만드는 중…', en: 'Creating…' },
  'op.dash.branch.addto': { ko: '내 트랙에 지식 추가', en: 'Add knowledge to one of your tracks' },
  'op.dash.branch.pick': { ko: '트랙', en: 'Track' },
  'op.dash.branch.patchid': { ko: '지식 ID', en: 'Knowledge id' },
  'op.dash.branch.owner_note': { ko: '트랙은 관리자만 바꿀 수 있으며, 가장 최근 변경이 공개 기록에 남습니다.', en: 'Only the track owner can change it; the latest change is what the public record keeps.' },
};
...
export const operatorManage: Dict = {
  ...
  'op.manage.retire.crossbranch': { ko: '다른 트랙에 있는 겹침 {n}건은 그대로 남습니다 — 서로 다른 트랙의 지식은 함께 존재합니다.', en: '{n} overlap(s) on another branch stay as they are — knowledge on different branches coexists.' },
  ...
  // Item 367: who has my file, and why — the holder set exists in every manifest and was on no screen.
  'op.manage.holders.title': { ko: '이 파일을 가진 노드', en: 'Who holds this file' },
  'op.manage.holders.desc': { ko: '공개 기록과 연결된 노드 목록에서 이 파일(같은 해시)을 가지고 있다고 밝힌 노드입니다. 복제본이 많을수록 내 노드가 꺼져도 살아남지만, 가진 쪽이 누구인지도 함께 보여 줍니다.', en: 'Nodes that say they hold this exact file, from the public record and this node’s peer list. Copies are what keeps a knowledge alive when its seller is offline — and this says who has one.' },
  'op.manage.holders.empty': { ko: '아직 이 노드 말고는 이 파일을 가진 노드가 없습니다.', en: 'No other node holds this file yet.' },
  'op.manage.holders.count': { ko: '노드 {n}곳이 가지고 있습니다', en: 'held by {n} node(s)' },
  'op.manage.holders.split': { ko: '만든 노드 {authors} · 구매자 {buyers} · 검증 노드 {verifiers} · 복제본 {mirrors}', en: '{authors} publisher · {buyers} buyer(s) · {verifiers} verifier(s) · {mirrors} mirror(s)' },
  'op.manage.holders.node': { ko: '노드', en: 'Node' },
  'op.manage.holders.why': { ko: '가진 이유', en: 'Why it has it' },
  'op.manage.holders.why.author': { ko: '이 지식을 만들어 파는 노드입니다', en: 'the node that published it' },
  'op.manage.holders.why.buyer': { ko: '구매 기록이 있습니다', en: 'bought it — there is a settlement' },
  'op.manage.holders.why.verifier': { ko: '검증하려고 받았습니다', en: 'fetched it to verify — attestation on the record' },
  'op.manage.holders.why.mirror': { ko: '기록에 이유가 없는 복제본입니다', en: 'a copy with no purchase or attestation on the record' },
  'op.manage.holders.self': { ko: '이 노드', en: 'this node' },
  ...
  'op.manage.fields.branch': { ko: '트랙(브랜치)', en: 'Knowledge track' },
...
export const operatorLogs: Dict = {
  ...
  'op.logs.kind.branch': { ko: '트랙', en: 'track' },
...
export const operatorAccount: Dict = {
  ...
  'op.account.notif.all': { ko: '모두 — 판매, 검증 결과, 구독한 트랙에 새 지식', en: 'Everything — sales, verification results, new knowledge in subscribed tracks' },
  ...
  // Item 94 — the wallet is the only screen carrying settlement history, and it showed 20 rows of it with no count,
  // no way to the rest and nothing to reconcile against. Paging and a CSV of the WHOLE table, dates included.
  'op.account.rows.showing': { ko: '{total}건 중 {shown}건 보는 중', en: 'Showing {shown} of {total}' },
  'op.account.rows.more': { ko: '{n}건 더 보기', en: 'Show {n} more' },
  'op.account.rows.all': { ko: '{total}건 전부 보기', en: 'Show all {total}' },
  'op.account.rows.csv': { ko: 'CSV로 내려받기', en: 'Download CSV' },
  // Item 315 — what this node owes other people had no home but a tab about taught lessons, and its notice
  // described a transfer that a local ledger never makes.
  'op.account.owed': { ko: '이 노드가 원작자·데이터 제공자에게 줘야 할 몫', en: 'What this node owes creators and data providers' },
  'op.account.owed.desc.local': { ko: '이 노드는 로컬 기록(개발용)으로 정산합니다. 판매가 기록되는 순간 상대 잔액에 그대로 반영되므로 따로 보낼 것이 없습니다.', en: 'This node settles on a local record: each share is credited the moment the sale is written, so there is nothing to send.' },
  'op.account.owed.desc.ain': { ko: '판매마다 자동으로 이체를 시도하고, 실패하면 1분마다 최대 {n}회까지 다시 시도합니다. 개별 이체와 “다시 시도”는 내 지식 → 가르치기 탭에 있습니다.', en: 'Each share is transferred automatically after the sale, and a failed transfer retries every minute up to {n} times. Individual transfers and Retry live on My knowledge → Teaching.' },
  'op.account.owed.totals': { ko: '아직 보내지 못함 {open} · 잔액에 반영됨 {credited} · 이체 완료 {paid}', en: 'Not sent yet {open} · credited {credited} · transferred {paid}' },
  'op.account.owed.to': { ko: '받는 사람', en: 'To' },
  'op.account.owed.why': { ko: '사유', en: 'Why' },
  'op.account.owed.sales': { ko: '판매 건수', en: 'Sales' },
  'op.account.owed.why.lineage': { ko: '{id}의 원본을 만든 사람', en: 'creator behind {id}' },
  'op.account.owed.why.provider': { ko: '{id}의 데이터 제공자', en: 'data provider of {id}' },
  'op.account.owed.why.verification': { ko: '{id}을(를) 검증한 노드', en: 'verified {id}' },
  'op.account.owed.state.credited': { ko: '판매와 동시에 반영됨', en: 'credited with the sale' },
  'op.account.owed.empty': { ko: '아직 줘야 할 몫이 없습니다. 원본 지식이 있거나 데이터 제공자가 참여한 지식이 팔리면 여기에 나타납니다.', en: 'Nothing owed yet — a share appears here when knowledge with source creators or data providers sells.' },
  'op.account.owed.teaching': { ko: '가르친 수업에서 나온 몫만 따로 보고 이체를 다시 시도하려면', en: 'For the shares from taught lessons alone, and to retry a transfer:' },
  'op.account.owed.teaching.link': { ko: '내 지식 → 가르치기', en: 'My knowledge → Teaching' },
  // Item 356 — the wallet total is the biggest number on the screen and the only one with no provenance. The kind
  // of chain is read from the provider the node reports, not assumed to be the demo's local one.
  'op.account.wallet.network.local_chain': { ko: '이 노드가 붙어 있는 로컬 개발 체인의 AIN입니다 — 시장 가치가 없습니다.', en: 'AIN on the local development chain this node is attached to — it has no market value.' },
  'op.account.wallet.network.testnet': { ko: '테스트넷 AIN입니다 — 시장 가치가 없습니다.', en: 'Testnet AIN — it has no market value.' },
  'op.account.wallet.network.mainnet': { ko: 'AI Network 메인넷의 AIN입니다.', en: 'AIN on the AI Network mainnet.' },
  'op.account.wallet.network.unknown': { ko: 'AIN — 이 노드가 어느 체인에 붙어 있는지 알려 주지 않습니다.', en: 'AIN — this node does not say which chain it is attached to.' },
  'op.account.wallet.network.credit': { ko: '노드 크레딧 = 이 노드가 발행한 개발용 가상 화폐 (실제 가치 없음)', en: 'node credit = development play money issued by this node (no real value)' },
  ...
  'op.tech.context': { ko: '상황 속성 — 게이트웨이 라우터가 요청을 알맞은 트랙으로 보낼 때 씁니다', en: 'Context attributes — used by the gateway router to send requests to the right knowledge track' },
...
export const operatorTeach: Dict = {
  ...
  'op.teach.payouts.no_wallet': { ko: '이 노드에는 체인 지갑이 없어(로컬 기록) 보낼 이체 자체가 없습니다. 데이터 제공자 몫은 판매가 기록되는 순간 상대 잔액에 반영됩니다 — 계정 → 지갑에서 전부 볼 수 있습니다.', en: 'This node has no chain wallet (local record), so there is no transfer to make: each data-provider share is credited the moment the sale is recorded. Account → Wallet shows all of them.' },
