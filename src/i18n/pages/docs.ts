...
export const docs: Dict = {
  ...
  'docs.oneline.teach.help': { ko: '노드가 수업을 받는지, 공개 방식(검토/자동/비공개)과 대기열을 터미널에서 확인합니다. 가르치는 일 자체는 브라우저의 라이브 테스트에서 합니다 — 계정 없이, 틀린 답 아래의 "정답 가르치기"부터. 내려받은 수업 파일은 `ainize patch import`로 내 노드에 비공개 초안으로 넣을 수 있습니다.', en: 'Check from the terminal whether the node takes lessons, its publish mode (review / auto / never) and the queue. The teaching itself happens in the browser\'s Live test — no account, start from "Teach the right answer" under a wrong reply. A downloaded lesson goes onto your own node with `ainize patch import` as a private draft.' },
  'docs.oneline.teach.cta': { ko: '지금 가르치기 →', en: 'Teach now →' },
