/**
 * Minimal i18n (no dependency): `useT()` returns a translator bound to the current locale (ko default),
...
export function LocaleProvider({ children }: { children: ReactNode }) {
  const [locale, setLocaleState] = useState<Locale>(() => {
    ...
    return typeof navigator !== 'undefined' && navigator.language.startsWith('en') ? 'en' : 'ko';
  });
