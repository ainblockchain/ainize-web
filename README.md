...
| 3 | you | **Find someone's knowledge and use it.** Verified? → pay → download → load into the running model, no restart. *Buying is the operator's action (`ainize login` first) and only knowledge that finished verification is on sale; the browser live-tests it free but does not buy it.* | `ainize patch ls --node http://their-node:3402 --status LISTED -q "<topic>"`<br>`ainize login && ainize use <id>` | `/` → Explore knowledge → a knowledge page → Buy<br>`http://localhost:3402/explore` |
